...
export class ErixWorker<T = JsonValue> {
  ...
  private async pollForJobs(): Promise<void> {
    try {
      ...
      // Suppress errors caused by in-flight polls being aborted during
      // graceful shutdown — these are expected on scale-to-zero platforms
      // (Cloud Run, Fly.io) where SIGTERM cancels the active claim request.
