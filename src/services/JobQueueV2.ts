...
import type { BatchedJobWAL } from "./BatchedJobWAL.js";
...
export class JobQueueV2 extends EventEmitter {
	// ── In-memory state ───────────────────────────────────────────────────────
	/**
	 * Priority heap per queue: orders by (priority DESC, createdAt ASC).
	 * Replaces the previous Array.sort()-based approach for O(log n) claim.
	 * @requirements 2.1, 2.2, 2.5
	 */
	private queues = new Map<string, BinaryMaxHeap<Job>>();

	/** Job priority comparator: higher priority wins; on tie, older job wins (FIFO). */
	private static readonly JOB_COMPARATOR = (a: Job, b: Job): number => {
	...
	constructor(options: JobQueueOptions = {}, wal?: BatchedJobWAL) {
		super();
		this.options = {
	 ...
	 * Uses heap extraction with tenant fairness: extracts from the heap, skipping
	 * tenants at their parallel cap, and re-inserts skipped jobs.
	 * O(log n) amortized — only extracts/re-inserts jobs from capped tenants.
	 * Does NOT mark the job active — use `claim()` for that.
	 * @requirements 2.1, 2.2, 2.3, 2.4
	 */
	public pop(queueName: string): Job | null {
		const heap = this.queues.get(queueName);
		if (!heap || heap.size === 0) return null;

		// Extract jobs from the heap, skipping capped tenants.
		// Re-insert skipped jobs after we find an eligible one (or exhaust the heap).
		const skipped: Job[] = [];
		...
		while (heap.size > 0) {
			...
			// Tenant is at cap — skip this job and try the next one
			skipped.push(job);
		}

		// Re-insert all skipped jobs back into the heap
	...
	public getJob(jobId: string): Job | null {
		// Search the heap-based waiting queues
		for (const heap of this.queues.values()) {
			for (const job of heap) {
				if (job.id === jobId) return job;
			}
		}
		// Search array-based state maps
		const arrayMaps = [
			this.activeJobs,
			this.completedJobs,
			this.failedJobs,
	...
	public getMetrics(queueName: string): QueueMetrics {
		const waiting = this.queues.get(queueName)?.size ?? 0;
	...
	/**
	 * Cancel all intervals, flush the WAL, and remove all listeners.
	...
	public async destroy(): Promise<void> {
		...
		// Flush any buffered WAL entries before shutting down
		if (this.wal) {
	...
	private addToQueue(queueName: string, job: Job): void {
		if (!this.queues.has(queueName)) {
			this.queues.set(
				queueName,
				new BinaryMaxHeap<Job>(JobQueueV2.JOB_COMPARATOR),
			);
		}
