...
import { ErixStore } from "./Store.js";
...
export class PersistenceManager {
  ...
  async restore() {
    try {
      ...
      const { store, queues, rateLimits } = latest.data as any;
      if (store) this.store.importAll(store);
      if (queues) this.queue.import(queues);
      if (rateLimits) this.rateLimiter.import(rateLimits);
