...
import { JobWAL } from "./services/JobWAL.js";
...
async function bootstrap(): Promise<void> {
  try {
    ...
    queueV2.on("job:completed", (job) => console.log(`[Queue] ✓ ${job.id} (${job.queueName})`));
    queueV2.on("job:failed",    (job) => console.error(`[Queue] ✗ ${job.id} — ${job.error}`));
    queueV2.on("job:dlq",       (job) => console.error(`[Queue] DLQ ${job.id}`));
    queueV2.on("job:zombie",    (job) => console.warn(`[Queue] Zombie reaped: ${job.id}`));

    lock.on("lock:acquired", ({ key }: { key: string }) => {
      console.log(`[Lock] acquired: ${key}`);
    });
    cache.on("cache:evicted", ({ strategy, count }: { strategy: string; count: number }) => {
    ...
    const server = app.listen(PORT, () => {
      ...
      console.log(`   ├─ Job Queue       (WAL-backed, priority + DLQ + retry + heartbeat)`);
